...
const ROTATE_CHOICES = [
  ...
  { name: "يتغير 🔄 (يدور على كل الستايلات كل session)", value: "yes" },
] as const;
...
function buildCommands() {
  const timer = new SlashCommandBuilder()
    ...
    .addStringOption((o) =>
      o
        .setName(OPT_ROTATE)
        .setDescription("عايز الستايل يتغير كل session ولا يفضل ثابت؟")
        .setRequired(false)
        .addChoices(...ROTATE_CHOICES),
    );
...
async function handleInteraction(interaction: Interaction): Promise<void> {
  try {
    ...
    if (interaction.commandName === COMMAND_TIMER) {
      ...
      const rotateRaw = interaction.options.getString(OPT_ROTATE, false);
