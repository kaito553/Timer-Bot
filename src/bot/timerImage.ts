...
function ensureFont(): void {
  ...
  if (fs.existsSync(f)) {
    GlobalFonts.registerFromPath(f, "DejaVu");
    fontLoaded = true;
  }
}
...
const ALL_STYLES: TimerStyle[] = [
  "random", "hellokitty", "kuromi", "kaitokid", "gojo", "mylittlepony", "anime",
];

const W = 900, H = 400;

function formatTime(ms: number): string {
  const snapped = Math.floor(ms / 30000) * 30000;
...
function timeFontSize(timeStr: string): number {
  if (timeStr.length >= 7) return 115;
  if (timeStr.length >= 6) return 125;
  return 140;
}
...
function renderNeon(ctx: SKRSContext2D, opts: RenderOpts): void {
  ...
  ctx.strokeStyle = primary + "22"; ctx.lineWidth = 1;
  for (let x = 0; x < W; x += 40) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, H); ctx.stroke(); }
  for (let y = 0; y < H; y += 40) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke(); }

  for (const [cx, cy, r, col] of [
    [200, 100, 80, primary], [700, 300, 60, secondary], [450, 200, 40, accent],
  ...
  ctx.fillStyle = secondary; ctx.shadowColor = secondary; ctx.shadowBlur = 15;
  ctx.textAlign = "center";
  ctx.fillText(phaseText, W / 2, 80);

  const timeStr = formatTime(opts.remainingMs);
  ctx.font = `bold ${timeFontSize(timeStr)}px "DejaVu", sans-serif`;
  ctx.fillStyle = primary; ctx.shadowColor = primary; ctx.shadowBlur = 40;
  ctx.fillText(timeStr, W / 2, 240);

  drawProgressBar(ctx, opts, {
    x: 80, y: 300, width: W - 160, height: 18,
    color: primary, bgColor: primary + "33", glowColor: primary,
  });
...
function drawBow(ctx: SKRSContext2D, x: number, y: number, color: string, scale = 1): void {
...
function drawKittyFace(ctx: SKRSContext2D, cx: number, cy: number, r: number): void {
  ctx.fillStyle = "#fff"; ctx.strokeStyle = "#ffb3d1"; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.fill(); ctx.stroke();

  for (const [ex, ey] of [
    [cx - r * 0.65, cy - r * 0.8], [cx + r * 0.65, cy - r * 0.8],
  ] as [number, number][]) {
    ctx.fillStyle = "#fff";
    ctx.beginPath();
...
function renderHelloKitty(ctx: SKRSContext2D, opts: RenderOpts): void {
  ...
  ctx.fillStyle = "#ff4d94"; ctx.shadowColor = "#ff4d94"; ctx.shadowBlur = 8;
  ctx.textAlign = "center";
  ctx.fillText(opts.phase === "study" ? "🎀 Study Time 🎀" : "🎀 Break Time 🎀", 360, 80);
...
async function renderCharacterImage(
  ...
  drawProgressBar(ctx, opts, {
    x: 80, y: 345, width: W - 160, height: 14,
    color: charOpts.accentColor, bgColor: charOpts.accentColor + "44", glowColor: charOpts.accentColor,
  });
...
function drawSparkle(ctx: SKRSContext2D, x: number, y: number, color: string, size: number): void {
...
async function renderMLP(ctx: SKRSContext2D, opts: RenderOpts): Promise<void> {
  ...
  ctx.fillText(opts.phase === "study" ? "🦄 Study Time 🌈" : "🌸 Break Time 🌈", W / 2, 70);

  const timeStr = formatTime(opts.remainingMs);
  ctx.font = `bold ${timeFontSize(timeStr)}px "DejaVu", sans-serif`;
  ctx.strokeStyle = "rgba(255,255,255,0.85)"; ctx.lineWidth = 10; ctx.lineJoin = "round";
  ctx.shadowColor = "#ff80c0"; ctx.shadowBlur = 25;
  ctx.strokeText(timeStr, W / 2, 240);
  ctx.fillStyle = "#e91e8c"; ctx.shadowBlur = 18;
  ctx.fillText(timeStr, W / 2, 240);

  drawProgressBar(ctx, opts, { x: 80, y: 295, width: W - 160, height: 14, color: "#ff80c0", bgColor: "rgba(255,150,200,0.30)", glowColor: "#ff69b4" });
...
export async function renderTimerImage(opts: RenderOpts): Promise<Buffer> {
  ...
  let effectiveStyle: TimerStyle = opts.style;
  if (opts.rotate) {
    const startIdx = ALL_STYLES.indexOf(opts.style);
    const offset = startIdx >= 0 ? startIdx : 0;
    effectiveStyle = ALL_STYLES[(offset + opts.cycleCount) % ALL_STYLES.length];
  }
