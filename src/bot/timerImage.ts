...
function renderHelloKitty(ctx: ReturnType<typeof createCanvas>["getContext"], opts: RenderOpts) {
  ...
  // Draw Hello Kitty face (right side)
...
async function renderCharacterImage(
  ...
  const imgPath = path.join(ASSETS, imgFile);
  const img = await loadImage(imgPath);

  // Draw image filling canvas (cover)
  ...
  leftG.addColorStop(0, charOpts.overlayColor);
  ...
  rightG.addColorStop(0, charOpts.overlayColor);
  ...
  ctx.textAlign = "left";
  ctx.font = `bold 22px "DejaVu", sans-serif`;
  ctx.fillStyle = charOpts.accentColor;
  ctx.shadowColor = charOpts.accentColor; ctx.shadowBlur = 10;
...
async function renderMLP(ctx: ReturnType<typeof createCanvas>["getContext"], opts: RenderOpts) {
  ...
  // Frosted pill behind time
  const pill = { x:30, y:90, w:480, h:160, r:30 };
  ctx.save();
  ctx.beginPath();
  roundRect(ctx, pill.x, pill.y, pill.w, pill.h, pill.r);
  ctx.fillStyle="rgba(255,245,255,0.30)";
  ctx.fill();
  ...
  const label = opts.phase==="study" ? "🦄 My Little Study Time 🌈" : "🌸 My Little Break Time 🌈";
...
export async function renderTimerImage(opts: RenderOpts): Promise<Buffer> {
  ...
  const style = opts.style === "random" ? "random" : opts.style;
  ...
  switch (style) {
    ...
    case "kaitokid": {
      const kidIdx = ((opts.paletteIndex ?? 0) % 4) + 1;
      const kidExt = kidIdx <= 4 ? "jpg" : "png";
      await renderCharacterImage(ctx, opts, `kaitokid${kidIdx}.${kidExt}`, {
