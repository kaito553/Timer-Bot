...
const ROTATE_STYLES: TimerStyle[] = [
...
async function renderCharacterImage(
  ...
  ctx.fillStyle = charOpts.accentColor; ctx.shadowBlur = 6;
  ctx.fillText(`Cycle ${opts.cycleCount + 1}`, W / 2, 385);
  ctx.shadowBlur = 0;
}
...
export async function renderTimerImage(opts: RenderOpts): Promise<Buffer> {
  ...
  const effectiveStyle: TimerStyle = opts.rotate
    ? ROTATE_STYLES[opts.cycleCount % ROTATE_STYLES.length]
    : opts.style;
